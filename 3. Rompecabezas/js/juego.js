...
var instrucciones = [
  'Presionando las teclas ARRIBA, ABAJO, DERECHA, IZQUIERDA para mover las flechas',
  'Cuando termines de completar la imagen, ganaste',
];
...
function chequearSiGano () {
  ...
  for (var fila = 0; fila < cantidadFilas; fila++) {
    for (var col = 0; col < cantidadColumnas; col++) {
      if (!chequearPieza (fila, col)) return false;
    }
  }
  return true;

  // Validación
  function chequearPieza (fila, col) {
    return grilla[fila][col] === fila * cantidadFilas + col + 1;
  }
}

// Implementar alguna forma de mostrar un cartel que avise que ganaste el juego
function mostrarCartelGanador () {
  alert ('Felicitaciones, ha ganado el juego');
}
...
function posicionValida (fila, columna) {
  var filaCompleta = grilla.length - 1;
  var columnaCompleta = grilla[0].length - 1;
  return (
    fila >= 0 && fila <= filaCompleta && columna >= 0 && columna <= columnaCompleta
  );
}
